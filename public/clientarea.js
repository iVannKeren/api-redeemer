...
/** ✅ FIX BUG KECIL + TAMBAH METODE PEMBAYARAN */
function renderPayments() {
    // tampilkan metode pembayaran dari /api/payment-methods
    if (paymentMethodsBox) {
        if (!state.paymentMethods.length) {
            paymentMethodsBox.innerHTML = '<p class="text-slate-400 text-sm">Metode pembayaran belum diatur.</p>';
        } else {
            paymentMethodsBox.innerHTML = state.paymentMethods.map((m) => `
            ...
            : waiting
                ...
                : `
                    <div class="mt-3 flex gap-2 items-center">
                        ...
                        <button data-upload-proof-id="${o.id}" class="px-3 py-1 rounded bg-emerald-500 text-slate-950 text-xs ${canUpload ? '' : 'opacity-50 pointer-events-none'}">
                            Upload Bukti
                        </button>
                    </div>
                  `;
...
function renderLoadingState() {
    // optional: kamu bisa bikin spinner kalau mau
}
